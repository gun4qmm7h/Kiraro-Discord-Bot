...
from Kiraro import bot, human_format, get_hours
import json
...
async def rank(ctx, user: discord.Member = None):
    if not bool(user):
        user = ctx.author
    im1 = Image.open("Images/xp_stuff_with_border.png")
    ...
    im1.save("Images/Image_Rank.png")
    await ctx.send(file=discord.File("Images/Image_Rank.png"))
...
@voice_rank.error
async def voice_rank_error(ctx, error):
    if isinstance(error, discord.HTTPException):
        await ctx.send("Something went wrong, try again later")
    elif isinstance(error, commands.BadArgument):
        await ctx.send("Seems like I can't find that user")
...
async def stop_rank(ctx, string):
...
@stop_rank.error
async def stop_rank_error(ctx, error):
    if isinstance(error, discord.HTTPException):
        await ctx.send("Something went wrong, try again later")
...
@commands.has_permissions(administrator=True)
async def start_rank(ctx, string):
    if string.lower() in ["text", "txt", "t"]:
        with open("Files/Stop_Start_Rank.json") as f:
            stop_start = json.load(f)
...
@start_rank.error
async def start_rank_error(ctx, error):
    if isinstance(error, discord.HTTPException):
        await ctx.send("Something went wrong, try again later")
    elif isinstance(error, commands.MissingPermissions):
        embed = discord.Embed(
