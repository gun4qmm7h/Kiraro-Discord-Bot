...
@bot.command(aliases=['mutedrole', 'mrole', 'muted_role'])
@commands.has_permissions(manage_messages=True)
async def muterole(ctx, name=None, role_color=discord.Color.default()):
    try:
...
async def mute(ctx, user: discord.Member, time_mute=None):
    ...
    roles = discord.utils.get(ctx.guild.roles, id=Mute)

    if roles is None:
        await ctx.send("mute role is not set. To set it use the command `MuteRole`")
    elif user.guild_permissions.administrator:
        embed = discord.Embed(
            title="",
            description="You can't mute an `Administrator`",
            color=discord.Color.red()
        )
