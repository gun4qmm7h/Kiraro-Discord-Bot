...
async def OnlineSet(ctx, *, act_name):
    if await is_us(ctx.author.id):
        ...
        await ctx.send(f"I have changed my activity to `Online` with the status `{act_name}`")
...
async def DNDSet(ctx, *, act_name):
    if await is_us(ctx.author.id):
        ...
        await ctx.send(f"I have changed my activity to `Do Not Disturb` with the status `{act_name}`")
...
async def AwaySet(ctx, *, act_name):
    if await is_us(ctx.author.id):
        ...
        await ctx.send(f"I have changed my activity to `Away` with the status `{act_name}`")
